...
export default async function createRepository(
  targetOrgName = github?.context?.payload?.repository?.owner?.login
): Promise<void> {
...
async function callGithubEndpoint({
  targetOrgName,
  targetRepoName,
  ghToken,
  isPrivate,
...
function generateCreateRepoRequest({
  targetOrgName,
  targetRepoName,
  ghToken,
  isPrivate,
}: Inputs): AxiosRequestConfig {
